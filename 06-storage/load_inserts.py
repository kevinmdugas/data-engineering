...
def createTable(conn, is_temp_table=False):
    tbl = TableName
    tblcmd = f"TABLE {tbl}"
    if is_temp_table:
        tbl = TempTableName
        tblcmd = f"UNLOGGED TABLE {tbl}"
    with conn.cursor() as cursor:
        ...
        if not is_temp_table:
            ...
            with conn.cursor() as cursor:
                ...
                """)
            print("Finised adding primary key constraint and index to table")
        print(f"Created {tbl}")
...
def load(conn, icmdlist):
    ...
    with conn.cursor() as cursor:
        ...
        print(f'Finished Loading into temp table. Elapsed Time: {elapsed:0.4} seconds')

        
def appendTable(conn):
    with conn.cursor() as cursor:
        print("Appending temp table to reguar table")
        cursor.execute(f"""
...
def main():
    ...
    createTable(conn, is_temp_table=True)
    load(conn, cmdlist)
